...
public abstract class ForwardingSSTableReader extends SSTableReader {
    ...
    public ForwardingSSTableReader(SSTableReader delegate) {
        super(delegate.descriptor, SSTable.componentsFor(delegate.descriptor),
                TableMetadataRef.forOfflineTools(delegate.metadata()), delegate.maxDataAge,
                delegate.getSSTableMetadata(),
                delegate.openReason, delegate.header, delegate.indexSummary, delegate.dfile, delegate.ifile,
                delegate.bf);
    ...
    @Override
    protected RowIndexEntry getPosition(PartitionPosition key, Operator op, boolean updateCacheAndStats,
            boolean permitMatchPastLast, SSTableReadsListener listener) {
        return delegate.getPosition(key, op, updateCacheAndStats, permitMatchPastLast, listener);
    }

    @Override
    public UnfilteredRowIterator rowIterator(DecoratedKey key, Slices slices, ColumnFilter selectedColumns,
            boolean reversed, SSTableReadsListener listener) {
        return delegate.rowIterator(key, slices, selectedColumns, reversed, listener);
    }

    @Override
    public UnfilteredRowIterator rowIterator(FileDataInput file, DecoratedKey key, RowIndexEntry indexEntry,
    ...
    @Override
    public UnfilteredRowIterator simpleIterator(FileDataInput file, DecoratedKey key, RowIndexEntry indexEntry,
    ...
    @Override
    public UnfilteredPartitionIterator partitionIterator(ColumnFilter columns, DataRange dataRange,
    ...
    public boolean isReplicationTransferredToErasureCoding() {
        return delegate.isReplicationTransferredToErasureCoding();
    }
