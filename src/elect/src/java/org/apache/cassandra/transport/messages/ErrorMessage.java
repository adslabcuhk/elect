...
public class ErrorMessage extends Message.Response {
            ...
            switch (code) {
                ...
                case READ_FAILURE: {
                    ...
                    if (version.isGreaterOrEqualTo(ProtocolVersion.V5)) {
                        ImmutableMap.Builder<InetAddressAndPort, RequestFailureReason> builder = ImmutableMap
                                .builderWithExpectedSize(failure);
                        for (int i = 0; i < failure; i++) {
                            InetAddress endpoint = CBUtil.readInetAddr(body);
                            RequestFailureReason failureReason = RequestFailureReason
                                    .fromCode(body.readUnsignedShort());
                    ...
                    if (code == ExceptionCode.WRITE_FAILURE) {
                        ...
                        te = new ReadFailureException(cl, received, blockFor, dataPresent != 0,
        ...
        public void encode(ErrorMessage msg, ByteBuf dest, ProtocolVersion version) {
            ...
            switch (err.code()) {
                ...
                case READ_FAILURE: {
                    ...
                    if (version.isGreaterOrEqualTo(ProtocolVersion.V5)) {
                        for (Map.Entry<InetAddressAndPort, RequestFailureReason> entry : rfe.failureReasonByEndpoint
                                .entrySet()) {
        ...
        public int encodedSize(ErrorMessage msg, ProtocolVersion version) {
            ...
            switch (err.code()) {
                ...
                case READ_TIMEOUT:
                    ...
                    // CasWriteTimeoutException already implies protocol V5, but double check to be
                    // safe.
                    if (isWrite && version.isGreaterOrEqualTo(ProtocolVersion.V5)
     ...
     * @param unexpectedExceptionHandler a callback for handling unexpected
     *                                   exceptions. If null, or if this
     *                                   returns false, the error is logged at ERROR
     *                                   level via sl4fj
     */
    public static ErrorMessage fromException(Throwable e, Predicate<Throwable> unexpectedExceptionHandler) {
        int streamId = 0;

        // Netty will wrap exceptions during decoding in a CodecException. If the cause
        // was one of our ProtocolExceptions
