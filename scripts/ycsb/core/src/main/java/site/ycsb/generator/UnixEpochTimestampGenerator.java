 ...
 * data at 1 minute intervals, then to set the start timestamp an hour in the
 * past
 ...
 * <p>
 * WARNING: This generator is not thread safe and should not called from
 * multiple
 * threads.
 */
public class UnixEpochTimestampGenerator extends Generator<Long> {
   ...
   *                       NOTE that this must match the time units used for the
   *                       interval.
   *                       If the units are in nanoseconds, provide a nanosecond
   *                       timestamp {@code System.nanoTime()}
   *                       or in microseconds, {@code System.nanoTime() / 1000}
   *                       or in millis, {@code System.currentTimeMillis()}
   *                       or seconds and any interval above,
  ...
  /**
   * Sets the starting timestamp to the current system time plus the interval
   * offset.
